...
- Add `Sequence.reset/0` for reseting sequences between tests. ([#151])

### Changed

- `params_*` functions now drop fields with `nil` values ([#148])
- Don't delete `has_many`s from `params_*` functions ([#148])
...
[#151]: https://github.com/thoughtbot/ex_machina/pull/168
